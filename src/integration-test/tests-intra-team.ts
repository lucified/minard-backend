...
export default (
  ...
  describe('projects', () => {
    ...
    it('should be able to edit a project', async function() {
      ...
      const project = await client
        .editProject({ description: newDescription })
        .then(x => x.getEntity());
      expect(project.id).to.exist;
  ...
  describe('configuring notifications', () => {
    function testNotificationConfiguration(
      ...
      switch (configuration.type) {
        case 'flowdock':
          ...
          break;
        case 'hipchat':
          expect(attributes['hipchat-room-id']).to.eq(
            configuration.hipchatRoomId,
          );
          expect(attributes['hipchat-auth-token']).to.eq(
            configuration.hipchatAuthToken,
          );
          break;
        case 'slack':
          expect(attributes['slack-webhook-url']).to.eq(
        ...
        if (notificationConfiguration) {
          ...
          for (const scopedConfiguration of scopes) {
            const responseJson = await client
              ...
              .then(x => x.getEntity());
            const id = testNotificationConfiguration(
              scopedConfiguration,
              responseJson,
            );
            createdNotificationConfigurations[String(id)] = {
              id,
              ...scopedConfiguration,
            };
          }
    ...
    it('should be able to list configured notifications', async function() {
      ...
      const teamConfigurations = await client
        .getTeamNotificationConfigurations(teamId)
        .then(x => x.getEntities());
      const projectConfigurations = await client
        .getProjectNotificationConfigurations(projectId)
        .then(x => x.getEntities());

      // Assert
      const receivedConfigurations = teamConfigurations.concat(
        projectConfigurations,
      );
      expect(receivedConfigurations.length).to.eq(
        Object.keys(createdNotificationConfigurations).length,
      );
      for (const responseJson of receivedConfigurations) {
        const id = responseJson.id;
        testNotificationConfiguration(
          createdNotificationConfigurations[id],
          responseJson,
        );
      }
    });
  ...
  describe('comments', () => {
    ...
    it('should be able to add comment for deployment', async function() {
      ...
      comment = await client
        .addComment(client.lastDeployment!.id, message, name, email)
        .then(x => x.getEntity());
    ...
    it('should be able to fetch comments for deployment', async function() {
      ...
      const comments = await client
        .getComments(client.lastDeployment!.id)
        .then(x => x.getEntities());
      expect(comments.length).to.equal(1);
      expect(comments[0].attributes.message).to.equal(
  ...
  describe('removing notification configuration', () => {
    it('should be able to delete created configurations', async function() {
      ...
      for (const id of Object.keys(createdNotificationConfigurations)) {
        const response = await client.deleteNotificationConfiguration(
          Number(id),
        );
        expect(response.status).to.eq(200);
      }
  ...
  describe('realtime', () => {
      ...
      it('should be able to get realtime events', async function() {
        ...
        for (let k = 0; k < numEvents; k++) {
          ...
          const eventStream = withPing(
            await client.teamEvents(eventType),
            1000,
          ...
          // Act
          const editPromise = client
            .editProject({ description: newDescription })
            .then(x => x.getEntity());
          const [editResponse, sseResponse] = await Promise.all([
            editPromise,
            eventPromise,
          ]);
    ...
    describe.skip('deployment scoped events', () => {
      ...
      it('should be able to request events retrospectively', async function() {
        ...
        const sseResponse = await withPing(
          await client.deploymentEvents(
            eventType,
            deployment.id,
            deployment.token,
            eventResponses[0].lastEventId,
