...
import { ScreenshotModule, Screenshotter } from './';
...
describe('screenshot-module', () => {
  ...
  describe('takeScreenshot', () => {
    ...
    it('should call webshot with correct arguments', async () => {
      ...
      // Act
      const publicUrl = await screenshotModule.takeScreenshot(
        projectId,
        deploymentId,
        'foo',
      );
      ...
      expect(publicUrl).to.equal(
        `${baseUrl}/screenshot/${projectId}/${deploymentId}` +
