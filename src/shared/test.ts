...
export type MethodStubber<T> = (plugin: T, kernel: Container) => sinon.SinonStub | sinon.SinonStub[];
export const stubber = <T>(methodStubber: MethodStubber<T>, injectSymbol: symbol, kernel: Container) => {
  const instance = kernel.get<T>(injectSymbol);
  const stubs = (new Array <sinon.SinonStub>()).concat(methodStubber(instance, kernel));
